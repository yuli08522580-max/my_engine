...
int main() {
    ...
    if (!tryRemoveBlock(blocks, 0.29f, -0.21f, 0.1f)) return 10;
    if (blocks.size() != 1) return 11;
    if (!nearlyEqual(blocks[0].x, 0.1f) || !nearlyEqual(blocks[0].y, -0.2f)) return 12;

    if (tryRemoveBlock(blocks, 0.31f, -0.19f, 0.1f)) return 13;
    if (blocks.size() != 1) return 14;
